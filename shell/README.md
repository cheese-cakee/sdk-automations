...
> verify before accept, accept before ack, decide before act, report always.
...
| ④ One verb, then persist | [`src/processor.ts`](src/processor.ts), [`src/reports.ts`](src/reports.ts) | core's `decide()`; the shell cannot assert a world — `DerivedWorld` has no public constructor |
...
REPORTS_FILE=…              # optional; default data/decisions.jsonl
...
`data/decisions.jsonl` gains a line saying what the platform would have done and why. That file is
dry-run's entire product — the interview artifact.
...
The capture receiver in `lab/src/capture.ts` was this package's embryo: same verify-first line,
