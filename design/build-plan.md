...
- The roadmap does not test the capability-boundary principle (register P3) *with shipped capabilities*.
  ...
  **Amended 2026-08-05 (D70).** The reasoning above is right about the arithmetic and wrong about the
  ...
  capabilities would, and the run no longer waits for November: `packages/probes/test/engine-matrix.test.ts` covers
  all eight subsets of three deliberately dissimilar capability stubs (D70). P3 is now a tested claim in code
  against the boundary in `packages/core/src/capability/`; what remains for after November is re-running the matrix
