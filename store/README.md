...
| `effect_journal` | intent/done write-ahead rows with a durable per-call attempt counter; the recovery loop's detector | crash-proven in the 6.5 sandbox grid; attempt counter and done-immutability added under D42 |
...
| `schedule` | clock-triggered work; `pending → running → done`, `claimed_at` stamped on claim so the sweep can requeue stuck rows | decided in 6.5; exactly-once-across-restart is a stage-five exit-gate test, pre-covered here; `claimed_at` added under D43 |
...
- `FINDING(store-journal-attempts)` → **D42** — `done` rows are
  ...
  so retry bounds survive restart.
...
22.x, runs unflagged from 23.4, and is stable in 24. Part of the repository's pnpm
