...
3. **Conformance (when the adapter ships)** — scheduled probes re-verify the perishable facts in
   `core/src/github/` and stamp a tracked result file a `checks/` lock reads.

Tracked: `protocols/`, `src/`, `test/`. Never tracked: `harness/` (the era-1 code and private
evidence archive), `evidence/` (capture staging), `.env` — enforced by `checks/test/lab.test.ts`,
...
flowchart LR
    ...
    pending -->|"a human reads every file"| fixtures["core/test/github/fixtures/ — for events.ts"]
...
- [ ] **Reviewed captures land directly in `core/test/github/fixtures/`** — no waypoint: the
...
- [`endpoint-permission-matrix.md`](endpoint-permission-matrix.md) — one
  row per operation: endpoint, permission, observed behavior.
- [`storage-decision.md`](storage-decision.md) — the three recovery
