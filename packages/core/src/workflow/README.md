...
flowchart TB
    ...
    CAU --> TRA
    CAU --> STA
    TRA --> APP["apply.ts — may this move, and what results"]
    STA --> APP
...
| [`transitions.ts`](transitions.ts) | Which moves are legal? The two diagrams, verbatim as edge tables. |
| [`apply.ts`](apply.ts) | May this move happen, and what does the item look like afterwards? |
...
conflicted item has no `WorkItemState`, so it cannot reach `apply.ts` at all. The no-write rule is
...
`apply.ts` has **no production callers, deliberately**. It is the executable taxonomy spec: a test
oracle today, and the adapter's read-back conformance checker when that lands (D93).
